...
class S99$Test extends FunSuite {
  ...
  test("P14") {
    assert(S99.duplicate(Nil) == Nil)
    assert(S99.duplicate(List('a, 'b, 'c, 'c, 'd)) == List('a, 'a, 'b, 'b, 'c, 'c, 'c, 'c, 'd, 'd))
    assert(S99.duplicate(List('a, 'a, 'b)) == List('a, 'a, 'a, 'a, 'b, 'b))
  }

  test("P15") {
    assert(S99.duplicateN(0, Nil) == Nil)
    assert(S99.duplicateN(0, List('a, 'b, 'c)) == Nil)
    assert(S99.duplicateN(1, Nil) == Nil)
  ...
  test("P18") {
    ...
    assert(S99.slice(3, 7, List('a)) == Nil)
  }

  test("P19") {
    assert(S99.rotate(3, List('a, 'b, 'c, 'd, 'e, 'f, 'g, 'h, 'i, 'j, 'k)) == List('d, 'e, 'f, 'g, 'h, 'i, 'j, 'k, 'a, 'b, 'c))
    assert(S99.rotate(-2, List('a, 'b, 'c, 'd, 'e, 'f, 'g, 'h, 'i, 'j, 'k)) == List('j, 'k, 'a, 'b, 'c, 'd, 'e, 'f, 'g, 'h, 'i))
    assert(S99.rotate(-2, Nil) == Nil)
  }
}
